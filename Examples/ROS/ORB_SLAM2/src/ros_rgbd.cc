...
{
public:
    ImageGrabber(ORB_SLAM2::System* pSLAM):subCnt(0),relocate(false),mpSLAM(pSLAM),
    rgb_sub(n, "/camera/rgb/image_raw", 1),
    depth_sub(n, "/camera/depth_registered/image_raw", 1),
    ...
    bool SaveMapStatus(ORB_SLAM2::SaveMap::Request &req,ORB_SLAM2::SaveMap::Response &res);
    bool RelocationStatus(ORB_SLAM2::Relocation::Request  &req,                            
                          ORB_SLAM2::Relocation::Response &res);                           
    int subCnt;                              
    bool relocate;
...
bool ImageGrabber::SaveMapStatus(ORB_SLAM2::SaveMap::Request  &req, 
                                 ORB_SLAM2::SaveMap::Response &res) 
{                                                            
  res.result = req.name;                                           
  //ROS_INFO("request: bool=%s", req.a.c_str());                 
  ROS_INFO("save map response:%s", res.result.c_str());    
  //if(res.sum)                                                
    mpSLAM->SaveMap("Map.map");                                

  return true;                                               
}                                                            
bool ImageGrabber::RelocationStatus(ORB_SLAM2::Relocation::Request  &req,  
                                    ORB_SLAM2::Relocation::Response &res)            
{                                                                          
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceServer map_service = nh.advertiseService("vslam/save_map", &ImageGrabber::SaveMapStatus,&igb);          
