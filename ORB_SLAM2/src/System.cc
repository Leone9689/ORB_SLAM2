		      	   ...
		      	   mbDeactivateLocalizationMode(false)
{
    ...
    map = new Map(); 
    if(mapName != "none")    
      bMapLoad = map->Load(mapName, *mpVocabulary);//载入map信息
    if(!bMapLoad || (mapName=="none"))                                                    
    {                                                                
      delete map;                                                    
      map=NULL;                                                      
      cerr << "No such map file. " << endl;                               
    }                                                                
    //Create the Map
    if (map==NULL) 
      mpMap = new Map();
...
void System::Initialize(bool pMapLoad,string pMapName)
{
    bool bMapLoad;
    mpMap = new Map(); 
    if(pMapName != "none")
    {    
      bMapLoad = mpMap->Load(pMapName, *mpVocabulary);//载入map信息
      if(!bMapLoad )                                                    
      {                                                                
        //delete map;                                                    
        //map=NULL;                                                      
    ...
    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    mpTracker = new Tracking(this, mpVocabulary, mpMap, mpKeyFrameDatabase, settingsFile, mSensor);
